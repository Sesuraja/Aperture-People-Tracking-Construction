...
mongodbRouter.post('/test-connection', async (req: Request, res: Response) => {
  ...
  const result = await testMongoConnection(mongodbUri);
...
mongodbRouter.post('/config', async (req: Request, res: Response) => {
  ...
  if (result.success) {
    return res.json({ success: true, connected: true, message: 'MongoDB connection string updated and connected successfully.' });
  } else {
    return res.status(500).json({ success: false, connected: false, error: result.error || 'Failed to connect with provided MongoDB connection string' });
  }
});
