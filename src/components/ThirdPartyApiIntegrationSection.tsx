...
export default function ThirdPartyApiIntegrationSection() {
  ...
  const fetchApis = async () => {
    try {
      setLoading(true);
      const res = await fetch("/api/integrations/third-party");
  ...
  const handleDeleteApi = async (id: string) => {
    ...
    try {
      const res = await fetch(`/api/integrations/third-party/${id}`, { method: "DELETE" });
      const data = await res.json();
      if (data.success) {
        await fetchApis();
        if (selectedApiId === id) {
  ...
  const handleTestConnection = async () => {
    try {
      ...
      setTestResult(null);
      setSyncNotice(null);
      const res = await fetch("/api/integrations/third-party/test", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(formData)
      });
  ...
  const handleSyncNow = async () => {
    try {
      setIsSyncing(true);
      setSyncNotice(null);
      const res = await fetch("/api/integrations/third-party/sync", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
